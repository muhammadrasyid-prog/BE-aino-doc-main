...
// GetSortedProduct mengikuti logika GetAllProduct dengan tambahan sorting
func GetSortedProduct(sortColumn, sortDirection string) ([]models.Product, error) {
    products := []models.Product{}

    // Filter kolom dan arah sorting untuk keamanan
    if sortColumn != "product_name" && sortColumn != "product_owner" {
    ...
    rows, errSelect := db.Queryx("SELECT product_uuid, product_name, product_owner, product_order, created_by, created_at, updated_by, updated_at FROM product_ms WHERE deleted_at IS NULL ORDER BY " + sortColumn + " " + sortDirection)
    if errSelect != nil {
        return nil, errSelect
    }
    defer rows.Close()
...
func UpdateProduct(updateProduct models.Product, id string, username string) (models.Product, error) {
	...
	if err != nil {
		log.Print("Error updating tour guide", err)
		return models.Product{}, err
	}
