...
func GetSortedProduct(c echo.Context) error {
    // Menerima parameter sorting dari query
    sortColumn := c.QueryParam("sort")
    sortDirection := c.QueryParam("direction")

    // Daftar kolom yang diizinkan untuk sorting
    validSortColumns := map[string]bool{
        "product_name":  true, // Nama Produk
        "product_owner": true, // PIC
    }

    // Menentukan kolom default untuk sorting
    if sortColumn == "" {
        sortColumn = "product_name" // Misalnya, default ke Nama Produk
    } else if !validSortColumns[sortColumn] {
        return c.JSON(http.StatusBadRequest, models.Response{
            Code:    400,
    ...
    // Menentukan arah default
    if sortDirection != "asc" && sortDirection != "desc" {
        sortDirection = "asc" // Arah default
    }

    // Panggil service dengan parameter sorting
    product, err := service.GetSortedProduct(sortColumn, sortDirection) // Panggilan fungsi yang benar
    if err != nil {
        log.Print(err)
        response := models.Response{
            Code:    500,
